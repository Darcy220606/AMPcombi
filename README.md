...
<span style="color:green"> (D) For filtering AMP hits: AMPcombi is developed to filter hits based on their AMP probabilities given by each AMP tool - E-values in case of HMMsearch - and the presence of stop-codons downstream and upstream of the AMP hit on the contig.
 ...
 ⚠️ If the user provides output files generated by [interproscan](https://github.com/ebi-pf-team/interproscan), hits with specific domains found can be further filtered out. An example is provided in the `parse_tables` section.
...
This subcommand parses and filters the output files generated by different AMP prediction tools, aligns the amino-acid sequences to different reference database and estimates the physicochemical annotations.
...
- **--interpropscan_output** a path to a directory or file that contains the results generated by running interproscan on the annotated sequences ('*.faa'). **NOTE:** The file names must match '<sample_name>.faa.tsv'. Additionally, coding sequences assigned as 'ribosomal proteins' are further removed using `--interproscan_filter 'ribosomal proteins,ribosomal'`. An example of how to run interproscan to prepare the files is shown below. 
...
| --interproscan_output | Path to the folder containing the output obtained from interproscan (i.e., in '*.faa.tsv'). This was only tested using the output from applications:[PANTHER,ProSiteProfiles,ProSitePatterns,Pfam]. | None | ./interproscan_output/ |
| --interproscan_filter | A comma separated list of all keywords that describes the protein that is not required in the analysis. | 'ribosomal protein,ribosomal proteins,ribosome protein,ribosomal rna,Ribosomal protein,RIBOSOMAL PROTEIN' | '16S' |
...
<details><summary>Example run for interproscan to generate input for '--interproscan_output'</summary>

[interproscan](https://interproscan-docs.readthedocs.io/en/latest/HowToRun.html)
- Download the interproscan datasets as described [here](https://interproscan-docs.readthedocs.io/en/latest/UserDocs.html#obtaining-a-copy-of-interproscan)
```
singularity pull docker://interpro/interproscan:latest

#If the sequences contain any non alphabet characters it will crash!
