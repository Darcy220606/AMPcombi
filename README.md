...
- Using **conda**:
```
conda create -n ampcombi python==3.11 diamond==2.0.15 mmseqs2==15.6f452 ampcombi
```
or 
```
...
This subcommand parses and filters the output files generated by different AMP prediction tools, aligns the amino-acid sequences to the reference database and estimates the physicochemical annotations. Additionally, given results generated by running [interproscan](https://github.com/ebi-pf-team/interproscan), any coding region that is classified as ribosomal protein can be removed by default. However, more keywords can be added using `--interproscan_filter 'keyword1,keyword2,keyword3'`. To get a full list of options available and their defaults please refer to the help documentation of the submodule
